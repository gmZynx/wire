...
if CLIENT then
	...
	function ENT:OnVarChanged(name, old, new)
		...
		elseif name == "Distance" then
			flashlight:SetFarZ(game.SinglePlayer() and new or math.Clamp(new, 64, 5000))
		elseif name == "Brightness" then
			flashlight:SetBrightness(game.SinglePlayer() and new or math.Clamp(new, 0, 100))
	...
	function ENT:Think()
		...
		if not self.Flashlight then
			...
			flashlight:SetFarZ(singleplayer and self:GetDistance() or math.Clamp(self:GetDistance(), 64, 5000))
			flashlight:SetFOV(singleplayer and self:GetFOV() or math.Clamp(self:GetFOV(), 0, 170))
			flashlight:SetBrightness(singleplayer and self:GetBrightness() or math.Clamp(self:GetBrightness(), 0, 100))
