...
local max_convar = CreateConVar( "wire_expression2_lights_max", 15, { FCVAR_ARCHIVE, FCVAR_NOTIFY } )
...
local function CreateLight( self, index, position, color, distance, brightness )
	...
	if light and light:IsValid() then
		...
		if distance then
			light:SetKeyValue("distance",math.Clamp(distance,0,2048))
			light.Size = distance
		end
		
		if brightness then
			light:SetKeyValue("brightness",math.Clamp(brightness,0,50))
		...
		light:SetKeyValue("brightness",math.Clamp(brightness,0,20))
		light.brightness = brightness
...
e2function void lightDistance( index, distance )
	local light = GetLight( self, index )
	if not light then return end
	distance = math.Clamp( distance, 0, 2048 )
...
e2function void lightBrightness( index, brightness )
	local light = GetLight( self, index )
	if not light then return end
	brightness = math.Clamp( brightness, 0, 50 )
